<script lang="ts">
	...
	let useragent: ReturnType<typeof UAParser>;
	...
	if ('navigator' in globalThis && 'userAgent' in navigator) {
		...
		console.warn('Could not retreive useragent!');
	}
	console.log('shouldFlipDimensions: ', shouldFlipDimensionsInPortrait);
	...
	function tick() {
		...
		if (!video || !ctx || !worker) return console.error('no video or ctx or worker');
	...
	async function setVideoStream() {
		console.debug('Setting video stream');
		...
		if (aspectRatio > 1) {
			...
			console.debug(`aspectRatio > 1, width=${idealVideoWidth} height=${idealVideoHeight}`);
		}
		...
		console.debug(videoConstraints);
		...
		video!.onloadedmetadata = async () => {
			console.debug('Video metadata loaded', video!.videoWidth, video!.videoHeight);
			video!.play();
		};
	}
	...
	async function startScan() {
		if (scanning || initScanning) return console.debug('Already scanning; ignoring startScan()');
		initScanning = true;
		await initWorker();
		try {
			setVideoStream();
	...
	function stopScan() {
		if (!scanning) return console.debug('Not scanning; ignoring stopVideo()');
